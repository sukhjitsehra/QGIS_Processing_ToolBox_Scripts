...
def create_views(dsn, dbprefix, viewprefix, hstore, columns, date):
    try:
      ...
      QMessageBox.critical(None, "About Layer", "Connection to DB failed")
    cur = con.cursor()
...
if mapnik.mapnik_version() >= 800:
    text = mapnik.TextSymbolizer(mapnik2.Expression('[Name]'),'DejaVu Sans Bold',12,mapnik2.Color('black'))
else:
