# Routing Distances and Direct distances using OSM Data and adding Google Map Distances
##[Sehra]=group
...
array(SELECT  distinct on (g2.gid) g1.id FROM {1}_vertices_pgr As g1, {0} As g2 order by  g2.gid, g1.the_geom <-> g2.geom ),
directed:=true) as X INNER JOIN {1}_vertices_pgr As Z ON X.start_vid = Z.id 
...
for row in result1:
    ...
    dis_mat = gmaps.distance_matrix(row[4],row[5],'driving')
    z= ast.literal_eval(json.dumps(dis_mat))
    gdist= z['rows'][0]["elements"][0]['distance']["text"]
    fet1.setAttributes([row[0],row[1],row[2], row[3],row[4],row[5],row[7],gdist])
