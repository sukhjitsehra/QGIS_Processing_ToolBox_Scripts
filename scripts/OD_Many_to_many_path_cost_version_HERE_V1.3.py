...
##pointTable=string ldh_points_utm43
...
FROM pgr_dijkstra(
'SELECT gid as id, source, target,  cost, reverse_cost_s as reverse_cost FROM ldh_here_mergedtwo',
...
FROM pgr_dijkstra(
'SELECT gid as id, source, target,  cost, reverse_cost_s as reverse_cost FROM ldh_here_mergedtwo',
array(SELECT  distinct on (g2.gid)  g1.id  FROM ldh_here_mergedtwo_vertices_pgr As g1, {0} As g2 order by  g2.gid, g1.the_geom <-> g2.geom ),
array(SELECT  distinct on (g2.gid) g1.id FROM ldh_here_mergedtwo_vertices_pgr As g1, {0} As g2 order by  g2.gid, g1.the_geom <-> g2.geom ),
directed:=true) as X INNER JOIN ldh_here_mergedtwo_vertices_pgr As Z ON X.start_vid = Z.id 
