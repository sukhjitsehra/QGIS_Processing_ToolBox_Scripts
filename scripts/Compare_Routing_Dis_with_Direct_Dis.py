...
# conn=psycopg2.connect(database=Db_points, host="localhost", user="postgres", password="a")
cur = conn.cursor()
...
(SELECT id FROM  ldh_here_mergedtwo_vertices_pgr ORDER BY the_geom <->
(SELECT geom from ldh_points_utm43 limit 1)  limit 1) ,
647 ,
FALSE
)  X INNER JOIN
ldh_here_mergedtwo_vertices_pgr AS Y ON X.node = Y.id LEFT JOIN
ldh_here_mergedtwo AS Z ON X.edge = Z.gid  and X.edge<>-1
ORDER BY seq;"""
cur.execute(sql)
result = cur.fetchall()
